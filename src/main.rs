...
async fn login(
    ...
    let (user_id, stored_hash, must_change_password, mfa_enabled) =
    ...
    if !mfa_enabled {
        return finish_login(&state, user_id, must_change_password).await;
    }

    // Password checked out, but a second factor is still owed -- no
    // session yet. Issue a short-lived pending token instead; the client
    // exchanges it (plus a TOTP code) at /mfa/login-verify.
